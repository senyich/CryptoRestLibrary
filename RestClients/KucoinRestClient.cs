...
public class KucoinRestClient : ExchangeRestClient
{
    ...
    public override async Task<OrderbookResponce> GetOrderbookAsync(string symbol, int limit = 20)
    {
        ...
        if (!response.IsSuccessStatusCode)
            throw new Exception($"[KucoinExchange]: http ошибка при получении ордербука по паре {symbol}");
    ...
    public override Task<WithdrawalDataResponce> GetWithdrawalDataAsync(string symbol)
    {
        throw new NotImplementedException();
    }
